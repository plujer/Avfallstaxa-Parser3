...
echo.
echo [6/28] Scannar masterarbetsbokens schema...
...
echo.
echo [7/28] Scannar och normaliserar standardtaxekatalog...
...
echo.
echo [8/28] Tolkar taxekoder...
python excel_builder_tax_codes.py --standard-tax "data\edp_standard\EDP_Future_Standard_Taxor_Renhallning.xlsx" --workbook "data\ArbetsExcel_Reference.xlsx" > output\excel\tax_code_intelligence_console.txt 2>&1

echo.
echo [9/28] Extraherar taxekunskap...
...
echo.
echo [10/28] Bygger Knowledge Index...
...
echo.
echo [11/28] Bygger semantiska taxaprofiler...
python excel_builder_semantic_profiles.py --parser-result "output\reports\parser3_result.json" --standard-tax "data\edp_standard\EDP_Future_Standard_Taxor_Renhallning.xlsx" --workbook "data\ArbetsExcel_Reference.xlsx" > output\excel\semantic_profile_console.txt 2>&1

echo.
echo [12/28] Rankar semantiska kandidater...
python excel_builder_semantic_candidates.py --parser-result "output\reports\parser3_result.json" --standard-tax "data\edp_standard\EDP_Future_Standard_Taxor_Renhallning.xlsx" --workbook "data\ArbetsExcel_Reference.xlsx" > output\excel\semantic_candidate_console.txt 2>&1

echo.
echo [13/28] Profilerar Arbets-Excel...
...
echo.
echo [14/28] Läser EDP-regelverk...
...
echo.
echo [15/28] Bygger Master Rule Repository...
...
echo.
echo [16/28] Skapar Arbets-Excel snapshot...
...
echo.
echo [17/28] Bygger Taxepunkter row plan...
...
echo.
echo [18/28] Kör Matching Engine preview...
...
echo.
echo [19/28] Skapar standardtaxeförslag...
...
echo.
echo [20/28] Bygger Arbets-Excel från parseroutput...
...
echo.
echo [21/28] Skriver Tax Knowledge till Arbets-Excel...
...
echo.
echo [22/28] Skriver standardtaxeförslag till Arbets-Excel...
...
echo.
echo [23/28] Kör gammal beslutsmotor för jämförelse...
...
echo.
echo [24/28] Kör semantisk beslutsmotor...
python excel_builder_decide_semantic.py --parser-result "output\reports\parser3_result.json" --reference-workbook "data\ArbetsExcel_Reference.xlsx" --standard-tax "data\edp_standard\EDP_Future_Standard_Taxor_Renhallning.xlsx" --workbook "output\excel\ArbetsExcel_byggd_fran_parser.xlsx" > output\excel\tax_decision_semantic_console.txt 2>&1

echo.
echo [25/28] Validerar att alla Word-taxor finns i Taxepunkter...
...
echo.
echo [26/28] Kör isolerad Sorsele projektkörning...
...
echo.
echo [27/28] Skapar standardiserad rapportzip...
