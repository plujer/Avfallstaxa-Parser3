...
echo.
echo [1/8] Rensar gamla rapporter i output-root...
python tools\clean_output_root.py > output\diagnostics\cleanup_report.txt 2>&1

echo.
echo [2/8] Arbets-Excel profile...
python run.py --profile-master --master "C:\PyProjects\data\Master.xlsx" > output\diagnostics\master_console.txt 2>&1

echo.
echo [3/8] Parser semantic/explain/architecture/missing-debug/trace...
...
echo.
echo [4/8] Parser diff mot Arbets-Excel...
