...
This component is intentionally independent of tax extraction. It walks the raw
DocumentBlock stream and keeps the current section active until a new valid section
heading appears.
"""
    ...
    def assign(self, blocks: list[DocumentBlock]) -> list[ContextBlock]:
        ...
        for block in blocks:
            ...
            if block.kind == "paragraph":
                group = self.rules.normalize_group(block.text, state.group)
            ...
            if block.kind == "table":
                ...
                for row in block.rows:
                    ...
                    group = self.rules.normalize_group(text, state.group)
                    if group != state.group:
                        state.group = group
